...
$datacol=array();$count=0; $filter_text=""; $dimF=array();
		...
		for($i=0;$i<count($xdim);$i++)
		{
			...
			if(isset($sdim[7]))	$dimF[$pType]=trim($sdim[7]);	
		}	
		...
		$igroup=str_replace('|',',',$igroup);
		if(!empty($filter_text)) $filter_text = " and $filter_text";
		$q="select $idcol,$colstr from $t where $dateQuery $filter_text $icondition group by $pri order by $sort_col $dir";
		//print_r($_GET);
		//die($q);
...
<?php  echo $dummy[$dummy_count]; ?>
 <?php $dummy_count++;} else if($order[$j]=="i") {?>
	  ...
	  <tr class="graph_row"  >
		...
		<tr class="graph_row"  >
        </td><td class="gr-sn-col" style="font-weight:bold"><?php for($j=0;$j<count($columns);$j++) { ?><a href="javascript:;"><?php if(array_search($columns[$j],$datacol) !==false) echo number_format(array_sum(${$columns[$j]})/count(${$columns[$j]}),2,".",","); else if($j==0)echo "AVERAGE"; ?>&nbsp;</a><?php if(isset($cbreak[$j]) && $cbreak[$j]=="k"  && $j<count($coldesc)-1){ ?></td><td class="gr-sn-col"  style="font-weight:bold"> <?php } ?> <?php  } ?></td></tr><?php } ?>
      </tbody> 
    </table>
</div>
</div>
